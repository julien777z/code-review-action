...
class TestAlreadyReviewed:
    ...
    def test_api_failure_raises(self, monkeypatch, mock_config) -> None:
        """Test that GitHub API failures fail the duplicate guard loudly."""

        exc = subprocess.CalledProcessError(1, ["gh", "api"], stderr="gh: Too Many Requests (HTTP 429)")
...
class TestResolveThreads:
    ...
    def test_uses_github_token_when_resolve_token_unset(self, monkeypatch, mock_config) -> None:
        """Test that resolve_threads uses the github token when no resolve token is set."""
